export default class CheckIcon extends HTMLElement {
    constructor() {
        super();
        this.attachShadow({ mode: 'open' })
    }
    connectedCallback() {
        this.render()
    }
    static get styles() {
        return /*CSS*/ `
            :host{
                display: flex;
                place-items: center;
                gap: .5rem;
                font-size: 0.875rem;
            }
            .svg-check{
                fill: #4CA154;
            }
        `;
    }
    render() {
        this.shadowRoot.innerHTML = /*HTML*/ `
            <style>${CheckIcon.styles}</style>
            <i>
                <svg  width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path style="fill: ${this.getAttribute("fill")}" class="svg-check" fill-rule="evenodd" clip-rule="evenodd"
                        d="M12 21C16.9706 21 21 16.9706 21 12C21 10.1666 20.4518 8.46124 19.5103 7.03891L12.355 14.9893C11.6624 15.7589 10.4968 15.8726 9.66844 15.2513L6.4 12.8C5.95817 12.4686 5.86863 11.8418 6.2 11.4C6.53137 10.9582 7.15817 10.8686 7.6 11.2L10.8684 13.6513L18.214 5.48955C16.5986 3.94717 14.4099 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z"
                    />
                </svg>
            </i>
            <span>${this.getAttribute("text")}</span>
        `;
    }
    disconnectedCallback() { }
}
globalThis.customElements.define("jv-check-icon", CheckIcon);